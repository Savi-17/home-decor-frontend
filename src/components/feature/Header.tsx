...
import { setIsModalOpen } from "../../store/slice/userSlice";
...
export default function Header() {
  ...
  const onLoginClick = () => dispatch(setIsModalOpen(true));
  ...
  const allCategory = async () => {
    try {
      const response = await axios.get("api/category/AllCategory");
      setCategory(response.data.final);
    } catch (err) {
      ...
      <nav className="bg-lavender-50 border-t border-gray-200 hidden md:block">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center space-x-8">
            ...
            >
              ...
              {showMegaMenu && (
                <div className="absolute left-0 top-full w-screen max-w-4xl bg-white shadow-xl border border-gray-200 z-50">
                  <div className="flex flex-row space-x-8 p-6 overflow-x-auto">
                    {!!category.length &&
                      category.map((cat: any) => (
                        <div key={cat}>
                          ...
                          <ul className="space-y-2">
                            {!!cat.child.length &&
                              cat.child.map((item: any, index: number) => (
            ...
            <Link
              to="/new-arrivals"
              className="py-4 text-gray-700 hover:text-lavender-600 font-medium"
            >
              New Arrivals
            </Link>
            <Link
            ...
            <Link
              to="/sale"
              className="py-4 text-red-600 hover:text-red-700 font-medium"
            >
              Sale
            </Link>
            <Link
